...
impl<Message: Send + Sync + Clone> P2PControlSender<Message> {
    fn new(
        sender: mpsc::Sender<P2PControl<Message>>,
        peers: Arc<RwLock<PeerMap<Message>>>,
        back_pressure: usize,
    ) -> P2PControlSender<Message> {
        P2PControlSender {
...
pub trait P2PConfig<
    Message: Version + Send + Sync + 'static,
...
pub struct P2P<
    Message: Version + Send + Sync + Clone + 'static,
    Envelope: Command + Send + Sync + 'static,
    ...
    > P2P<Message, Envelope, Config>
{
    ...
    pub fn new(
        ...
        Arc<P2P<Message, Envelope, Config>>,
        P2PControlSender<Message>,
    ) {
        ...
        (
            p2p,
            P2PControlSender::new(control_sender, peers, back_pressure),
        )
    }
    ...
    fn control_loop(&self, receiver: P2PControlReceiver<Message>) {
        while let Ok(control) = receiver.recv() {
            match control {
                ...
                P2PControl::Bind(addr) => match self.add_listener(&addr) {
        ...
        match source {
            PeerSource::Outgoing(a) => {
                if let PeerSource::Outgoing(a) = source {
                    if peers.read().unwrap().values().any(|peer| {
                                    ...
                                    if let Ok(msg) = self.config.unwrap(msg) {
                                        if let Some(version) = msg.is_version() {
                                            if locked_peer.version.is_some() {
                                                ...
                                                ban = true;
                                                debug!(
                                                    "misbehaving peer, repeated version peer={}",
                                                    pid
                                                );
                                                break;
                                            }
                                            if version.nonce == self.config.nonce() {
                                                ...
                                                ban = true;
                                                debug!(
                                                    "rejecting to connect to myself peer={}",
                                                    pid
                                                );
                                                break;
                                            } else {
                                                if version.version
                                                    ...
                                                    || !version.services.has(needed_services)
                                                {
        ...
        loop {
            ...
            for event in events.iter() {
                ...
                if let Some(server) = self.is_listener(event.token()) {
                    ...
                    spawn
                        .spawn(
                            self.add_peer(network, PeerSource::Incoming(server))
                                .map(|_| ()),
                        )
                    ...
                    if let Err(error) =
                        self.event_processor(event, pid, needed_services, iobuf.as_mut_slice())
                    {
                        use std::error::Error;
...
impl<Message> Peer<Message> {
    ...
    pub fn send(&self, msg: Message) -> Result<(), Error> {
        ...
        self.sender
            .send(msg)
            .map_err(|_| Error::Downstream("can not send to peer queue".to_owned()))?;
    ...
    fn reregister_write(&self) -> Result<(), Error> {
        if !self.writeable.swap(true, Ordering::Acquire) {
            ...
            self.poll.reregister(
                &self.stream,
                self.pid.token,
                Ready::writable() | UnixReady::error() | UnixReady::hup(),
                PollOpt::level(),
            )?;
        }
        Ok(())
    ...
    fn read_advance(&mut self, buf: &mut [u8]) -> Result<usize, io::Error> {
        if self.chunks.len() == 0 {
            ...
            while have < buf.len() {
                ...
                // copy those
                buf[have..have + available]
                    .copy_from_slice(&current[self.pos.1..self.pos.1 + available]);
                // move pointer
                self.pos.1 += available;
                // have more
                have += available;
    ...
    fn write(&mut self, buf: &[u8]) -> Result<usize, io::Error> {
        if buf.len() > 0 {
            ...
            if nc == 0 || (buf.len() + self.chunks[nc - 1].len()) > IO_BUFFER_SIZE {
                ...
                self.chunks
                    .push_back(Vec::with_capacity(max(buf.len(), IO_BUFFER_SIZE)));
                nc += 1;
            }
